...
import { Container, Row, Col, Card, Form, Button, Alert, Badge, Offcanvas, Dropdown } from "react-bootstrap";
import { FaHeart, FaRegHeart, FaChevronRight, FaShoppingCart, FaFilter } from "react-icons/fa";
import { db, collection, getDocs, query, where, limit, auth, addDoc, deleteDoc, doc } from "../firebase";
...
const SearchResultsPage = () => {
    ...
    // Fetch products & filter by query
    const fetchData = useCallback(async () => {
        try {
            ...
            // 1. Generate search term variations (singular/plural) for smart matching
            ...
            // 2. Targeted Firestore Search using array-contains-any
            let results = [];
            try {
                // We use limit to avoid huge payload, but high enough for good results
                const qTargeted = query(prodRef, where("searchkeywords", "array-contains-any", queryTerms));
                const sTargeted = await getDocs(qTargeted);
                results = sTargeted.docs.map((d) => ({ id: d.id, ...d.data() }));
            } catch (err) {
                console.warn("Targeted search failed, falling back to full scan", err);
            }

            // 3. Fallback: If targeted search yields nothing, do a broader scan with a limit to ensure quick loading
            if (results.length === 0) {
                const qFallback = query(prodRef, limit(300));
                const sFallback = await getDocs(qFallback);
                results = sFallback.docs.map((d) => ({ id: d.id, ...d.data() }));
            }

            // 4. Client-side strict filtering (exact word boundary matching)
            let filtered = results.filter(p => {
                const terms = Array.isArray(p.searchkeywords) ? p.searchkeywords.map(t => t.toLowerCase()) : [];
                // Combine all searchable text into one string for easy regex checking (omitting pattern as requested)
                const searchString = [p.name, p.name_lower, p.category, p.subcategory, ...terms].filter(Boolean).join(" ").toLowerCase();

                // For a product to match, it must satisfy at least one of the query terms as a distinct word/prefix
                // The \b (word boundary) ensures 'men' or 'mens' does not match the end of 'women' or 'womens'
            ...
            // Map details & format
            let formattedList = filtered.map(doc => {
            ...
            // Get categories
            const uniqueCat = [...new Set(formattedList.map(p => p.category).filter(Boolean))];
    ...
    const handleAgeGroupChange = (age) => {
        setSelectedAgeGroups(prev => prev.includes(age) ? prev.filter(x => x !== age) : [...prev, age]);
        setCurrentPage(1);
    };
    ...
    const toggleWishlist = async (p) => {
        ...
        if (isFavorited) {
            ...
            try {
                ...
                setWishlist(prev => ({ ...prev, [pid]: newDoc.id }));
            ...
            <Row className="g-4">
                ...
                <Col lg={9} xs={12}>
                    <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
                        <div>
                            <h2 className="fw-bold text-dark mb-1">{searchQuery ? `"${searchQuery}"` : t("allProducts", "All Products")}</h2>
                        ...
                        <div className="d-flex align-items-center gap-3 w-100-mobile justify-content-between">
                            ...
                            <div className="d-flex align-items-center gap-2 ms-auto">
                                ...
                                <Dropdown align="end">
                                    ...
                                    >
                                        {sortBy === "popularity" ? t("popularity", "Relevance") :
                    ...
                    {loading ? (
                        ...
                        <Alert variant="info" className="rounded-3 border-0 shadow-sm p-4 text-center">
                            {t("noProductsMatchFilters", "No products found. Please try a different search term or clear filters.")}
                        </Alert>
                    ) : (
                        <>
