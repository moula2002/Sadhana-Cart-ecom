...
import FilterOffcanvas from "../components/searchBar/FilterOffcanvas";
import "../components/searchBar/MobileSearchBar.css";
...
import { 
  GiPoloShirt, GiRunningShoe, GiMonsteraLeaf
} from "react-icons/gi";
...
const SearchPage = () => {
  ...
  useEffect(() => {
    // Mock categories for "Trending Now"
    setTrending([
  ...
  return (
    <div className="search-page-container">
      ...
      <div className="mobile-search-header bg-primary">
        <button className="icon-btn text-white" onClick={() => navigate(-1)}>
          <FaArrowLeft />
        </button>
        ...
        {!searchTerm ? (
          ...
          <div className="suggestions-list">
            {loading ? (
              ...
              suggestions.map((p) => (
                <div key={p.id} className="suggestion-row" onClick={() => handleSelect(p)}>
                  ...
                  <div className="suggestion-info">
                    ...
                    <div className="suggestion-category">{t("inCategory", "in")} {p.category?.toLowerCase() || 'fashion'}</div>
                  </div>
              ...
              <div className="p-4 text-center text-muted">{t("noResultsFor", "No results found for")} "{searchTerm}"</div>
            )}
          </div>
        )}
      </div>
