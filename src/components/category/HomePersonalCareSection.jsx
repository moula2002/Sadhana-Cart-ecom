// src/components/HomePersonalCareSection.jsx
...
import "./HomePersonalCareSection.css";

// 🎨 STYLE CONSTANTS (Red Theme with Dark Mode)
const PRIMARY_TEXT_COLOR = "#2d2d2d";
const PRIMARY_TEXT_COLOR_DARK = "#e9ecef";
const ACCENT_COLOR = "#e63946"; // Vibrant Red
const ACCENT_COLOR_DARK = "#ff6b6b"; // Brighter red for dark mode
const CARE_COLOR = "#ff4d4d"; // Bright Coral Red
const CARE_COLOR_DARK = "#ff8585"; // Brighter for dark mode
const SALE_COLOR = "#d32f2f"; // Dark Red
const SALE_COLOR_DARK = "#ef5350"; // Brighter for dark mode
const WHITE_COLOR = "#FFFFFF";
const PRODUCT_BG_COLOR = "#fff5f5"; // Very light red for cards
const PRODUCT_BG_COLOR_DARK = "#1a1a1a"; // Dark mode card background
const RED_BG_GRADIENT = "linear-gradient(135deg, #fff5f5 0%, #ffeaea 100%)";
...
const getCustomStyles = (isDarkMode) => ({
  sectionContainer: {
    ...
    boxShadow: isDarkMode ? 
      "0 20px 60px rgba(230, 57, 70, 0.1)" : 
      "0 20px 60px rgba(230, 57, 70, 0.1)",
    border: isDarkMode ? 
  ...
  productCard: {
    ...
    boxShadow: isDarkMode ? 
      ...
      "0 10px 25px rgba(230, 57, 70, 0.05)",
  ...
  imageContainer: (isMobile) => ({
    ...
    backgroundColor: isDarkMode ? "#2d2d2d" : "#fffafa",
    padding: "10px",
  }),
  productImage: {
    maxWidth: "100%",
  ...
  discountBadge: {
    ...
    top: "12px",
    right: "12px",
    backgroundColor: isDarkMode ? CARE_COLOR_DARK : CARE_COLOR,
    color: WHITE_COLOR,
    ...
    zIndex: 10,
    boxShadow: isDarkMode ? 
      "0 4px 12px rgba(255, 133, 133, 0.3)" : 
  ...
  brandText: {
    fontSize: "0.7rem",
    fontWeight: "700",
    color: isDarkMode ? "#ffabab" : "#e63946",
    marginBottom: "4px",
    letterSpacing: "1px",
  ...
  price: {
    fontSize: "1.35rem",
    fontWeight: "900",
    color: isDarkMode ? CARE_COLOR_DARK : CARE_COLOR,
    letterSpacing: "-0.5px",
  },
  originalPrice: { 
    fontSize: "0.85rem", 
  ...
  headerUnderline: {
    ...
    width: "120px",
    height: "15px",
    backgroundColor: isDarkMode ? "rgba(255, 107, 107, 0.2)" : "rgba(230, 57, 70, 0.1)",
    zIndex: -1,
    borderRadius: "4px",
  },
  viewDealButton: {
  ...
  exploreButtonHover: (isDarkMode) => ({
    backgroundColor: isDarkMode ? "#fff" : CARE_COLOR,
    color: isDarkMode ? ACCENT_COLOR_DARK : WHITE_COLOR,
    transform: "translateY(-3px)",
    boxShadow: isDarkMode ? 
      `0 10px 25px rgba(255, 133, 133, 0.6)` : 
      `0 10px 25px rgba(255, 77, 77, 0.4)`,
  }),
  redWatermark: {
    position: "absolute",
    bottom: "-30px",
    ...
    fontSize: "12rem",
    fontWeight: "900",
    color: isDarkMode ? "rgba(255, 107, 107, 0.05)" : "rgba(230, 57, 70, 0.03)",
    userSelect: "none",
    pointerEvents: "none"
  },
  subtitle: {
...
const handleCardMouseEnter = (e, isDarkMode) => {
  ...
  e.currentTarget.style.boxShadow = isDarkMode ? 
    ...
    "0 25px 50px rgba(230, 57, 70, 0.12)";
...
const getProductImageSource = (product) => {
  ...
  return "https://placehold.co/300x380/fff5f5/e63946?text=CARE";
};
...
const generateDummyProduct = (index) => {
  ...
  const originalPrice =
    basePrice <= finalPrice
      ? finalPrice + Math.floor(Math.random() * 300) + 200
      : basePrice;
  const careTypes = [
    "Organic Face Serum",
    "Vitamin C Cream",
  ...
  return {
    id: `care-red-${index}`,
    name: `${careTypes[index % careTypes.length]} ${index + 1}`,
    brand: "GLOW ESSENTIALS",
    price: finalPrice,
    originalPrice,
    image: `https://picsum.photos/seed/care${index}/300/300`,
  };
};
...
function HomePersonalCareSection() {
  ...
  useEffect(() => {
    const fetchPersonalCare = async () => {
      setLoading(true);
      try {
        const cached = localStorage.getItem("personalCareProductsRed");
        if (cached) {
        ...
        let data = snapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
          price: doc.data().price ? Number(doc.data().price) : 299,
        ...
        setProducts(data);
        localStorage.setItem("personalCareProductsRed", JSON.stringify(data));
      } catch (err) {
        setProducts(
  ...
  return (
    <Container 
      fluid 
      className="category-section personal-care" 
      style={{ 
        background: isDarkMode ? DARK_BG_GRADIENT : RED_BG_GRADIENT, 
        padding: "70px 0",
        transition: "background 0.3s ease"
      }}
    >
      <Container style={customStyles.sectionContainer} className="personal-care-section-container">

        {/* Header */}
        <div className="text-center mb-5">
          <h3 style={customStyles.header} className="personal-care-section-title theme-text">
            PERSONAL CARE <span style={{ color: isDarkMode ? CARE_COLOR_DARK : CARE_COLOR }}>REDEFINED</span>
            <div style={customStyles.headerUnderline}></div>
          </h3>
          <p className="mt-3 fs-6 fw-light d-none d-sm-block" style={customStyles.subtitle}>
            Indulge in our curated selection of premium beauty essentials.
          </p>
        </div>

        {loading ? (
          <div className="text-center py-5">
            <Spinner animation="border" style={{ color: isDarkMode ? CARE_COLOR_DARK : CARE_COLOR }} />
            <p className="mt-3 text-muted theme-text-secondary">Loading beauty essentials...</p>
          </div>
        ) : (
          <>
                  ...
                  <Col key={product.id}>
                    <Link to={`/product/${product.id}`} className="text-decoration-none d-block h-100">
                      <Card
                        ...
                        className="personal-care-card theme-card"
                      ...
                      >
                        ...
                        <div style={customStyles.imageContainer(isMobile)}>
                          <LazyLoadImage
                            ...
                            onError={(e) => (e.target.src = "https://placehold.co/300x380/fff5f5/e63946?text=CARE")}
                          />
                        </div>
                        <Card.Body className="text-start p-3 d-flex flex-column">
                          <p style={customStyles.brandText} className="personal-care-brand-text">
                            {product.brand}
                          </p>
                          <Card.Title 
                            style={customStyles.title} 
                            className="text-truncate personal-care-card-title theme-text"
                          >
                            {product.name}
                          </Card.Title>
                          <div className="d-flex align-items-center mt-auto">
                            <span style={customStyles.price} className="personal-care-price">
                              ₹{product.price}
                            </span>
                            {product.originalPrice > product.price && (
                              <span style={customStyles.originalPrice} className="personal-care-original-price theme-text-secondary">
                          ...
                          <Button
                            ...
                            className="personal-care-view-deal-btn theme-button"
            ...
            <div className="text-center mt-5">
              <Link to="/personal-care">
                <Button
                  ...
                  className="personal-care-explore-btn theme-button"
